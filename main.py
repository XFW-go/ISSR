...
def lowlight_train(lowlight_enhance):
    ...
    train_low_data_names = glob('../data/train/train_L/*.png')
    train_low_data_names.sort()

    train_high_data_names = glob('../data/train/train_H/*.png')
    train_high_data_names.sort()
    
    train_seg_data_names = glob('../data/train/train_M/*.png')
    train_seg_data_names.sort()
    
    assert len(train_low_data_names) == len(train_high_data_names)
    assert len(train_low_data_names) == len(train_seg_data_names)
    ...
    for idx in range(len(train_low_data_names)):
        ...
        seg_im = load_images(train_seg_data_names[idx])
        train_seg_data.append(seg_im)

    eval_low_data = []
    eval_high_data = []

    eval_low_data_name = glob('../data/val/val_L/*.png')
    eval_high_data_name = glob('../data/val/val_H/*.png')
    ...
    # Train DecompNet
    lowlight_enhance.train(train_low_data, train_high_data, train_seg_data, eval_low_data, eval_high_data, batch_size=args.batch_size, patch_size=args.patch_size, epoch=args.epoch, lr=lr, sample_dir=args.sample_dir, ckpt_dir=os.path.join(args.ckpt_dir, 'Decom'), eval_every_epoch=args.eval_every_epoch, train_phase="Decom")
    
    # Train SegNet
    lowlight_enhance.train(train_low_data, train_high_data, train_seg_data, eval_low_data, eval_high_data, batch_size=args.batch_size, patch_size=args.patch_size, epoch=args.epoch, lr=lr, sample_dir=args.sample_dir, ckpt_dir=os.path.join(args.ckpt_dir, 'Seg'), eval_every_epoch=args.eval_every_epoch, train_phase="Seg")
    
    # Train ReflectNet
    lowlight_enhance.train(train_low_data, train_high_data, train_seg_data, eval_low_data, eval_high_data, batch_size=args.batch_size, patch_size=args.patch_size, epoch=args.epoch, lr=lr, sample_dir=args.sample_dir, ckpt_dir=os.path.join(args.ckpt_dir, 'Denoise'), eval_every_epoch=args.eval_every_epoch, train_phase="Denoise")
    
    # Train RelightNet
    lowlight_enhance.train(train_low_data, train_high_data, train_seg_data, eval_low_data, eval_high_data, batch_size=args.batch_size, patch_size=args.patch_size, epoch=args.epoch, lr=lr, sample_dir=args.sample_dir, ckpt_dir=os.path.join(args.ckpt_dir, 'Relight'), eval_every_epoch=args.eval_every_epoch, train_phase="Relight")
...
def lowlight_test(lowlight_enhance):
    ...
    for idx in range(len(test_low_data_name)):
        ...
        test_high_im = load_images(test_low_data_name[idx].replace('_L','_H'))
